...
**Оригинальные файлы** (папка `_GENESIS/`) — дистрибутивы программы версий 2.61 и 2.77 с рабочего компьютера лазерной установки: готовые EXE-файлы, конфигурации оборудования, примеры 3D-объектов. Версия 2.61 также содержит оригинальные исходные файлы на C++ (Borland C++ 4.5, 16-bit DOS).

**Реконструированный исходный код версии 2.77** (папка `Flint 2.77 src/`) — исходный код, восстановленный на основе оригинального кода версии 2.61 и дизассемблирования EXE-файла версии 2.77 (IDA PRO, Ghidra, Capstone и др.). Позволяет скомпилировать работающий EXE из исходников.

**32-битная версия (Open Watcom)** (папка `Flint 2.77 (WTCM32) src/`) — порт программы на компилятор Open Watcom C++ с DOS-экстендером DOS/4GW. Снимает ограничение 16-bit версии на ~80 000 точек и позволяет работать с моделями до 8 000 000 точек.

**32-битная версия (Borland)** (папка `Flint 2.77 (BCC32) src/`) — порт программы на компилятор Borland C++ 4.5 32-bit. Альтернативный вариант 32-битной сборки по сравнению с версией Open Watcom.

**Документация** (папка `Help/`) — интерактивное HTML-руководство пользователя со скриншотами всех экранов, справочник функций и подробное описание работы оборудования и софта.
...
### Сборка и запуск версии 2.77 (32-bit, Borland C++ Powerpack for DOS)

32-битная версия компилируется компилятором Borland C++ 4.5 32-bit, который есть в репозитории (`Tools/BC45_32/`).

1. Запустите файл `Flint 2.77 (BCC32) src/COMPILE.bat`
2. Скрипт скомпилирует программу и соберёт папку `Flint 2.77 (BCC32) src/BUILD/`
3. Перейдите в созданную папку `BUILD/` и запустите `RUN.bat`

### Сборка и запуск версии 2.77 (32-bit, Open Watcom C++)

32-битная версия компилируется компилятором Open Watcom C++, который тоже есть в репозитории (`Tools/WATCOM/`).

1. Запустите файл `Flint 2.77 (WTCM32) src/COMPILE.bat`
2. Скрипт скомпилирует программу и соберёт папку `Flint 2.77 (WTCM32) src/BUILD/`
3. Перейдите в созданную папку `BUILD/` и запустите `RUN.bat`
...
│
├── _GENESIS/                         — оригинальные дистрибутивы с рабочего компьютера
...
│
├── Flint 2.77 (BCC32) src/           — порт версии 2.77 на Borland C++ 32-bit
│   ├── PROJ5.CPP, PROJ5.H           — основной код программы
│   ├── MSKEYC.CPP                    — работа с мышью и клавиатурой
│   ├── PIC.CPP                       — графика и интерфейс
│   ├── RDWRF.CPP                     — чтение и запись файлов
│   ├── COMPILE.bat                   — скрипт компиляции (Borland C++ 32-bit)
│   └── BUILD/                        — собранная программа
│
├── Flint 2.77 (WTCM32) src/          — 32-битный порт версии 2.77 (Open Watcom C++)
...
│   ├── HARDWARE_DESCRIPTION.md       — справочник аппаратной части (лазер, плата, порты)
│   ├── FUNCTIONS_REFERENCE.md        — справочник всех функций исходного кода
│   ├── LOG_REFERENCE.md              — справочник логирования и отладки
│   ├── docs/                         — вспомогательные материалы (PowerPack for DOS)
│   ├── hardware_diagram_scans/       — отсканированные схемы оборудования
...
└── Tools/                            — инструменты для сборки, запуска и анализа
    ...
    ├── BC45_32/                      — компилятор Borland C++ 4.5 для 32-bit (дополнительная версия)
    ├── WATCOM/                       — компилятор Open Watcom C++ (для 32-bit сборки)
    ├── DOSBox-X/                     — эмулятор DOS (для запуска на современном Windows)
    ├── fix_hotspots.py               — исправление позиций хотспотов в FLINT_Help.html
