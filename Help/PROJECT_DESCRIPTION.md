...
- Управляет насосом охлаждения и вентилем
...
**Управление насосом и затвором:**
- **Gate delay** — задержка управления вентилем (gate) в мс
- **Pump/Gate** — переключатели включения/выключения насоса и вентиля
- **Кнопка «X»** — немедленная остановка насоса и закрытие вентиля
- **dT** — отображение переменной задержки (используется в расчёте перемещения)
- **sh_delay** — задержка открытия затвора
...
- Программа проверяет, что все точки объекта не выходят за **пределы хода столика** (0–235 мм по X и Y, 0–120 мм по Z)
...
| Файл | Зачем нужен |
|------|-------------|
| [PROJ5.CPP](PROJ5.CPP) | **Главный файл программы.** Всё, что вы видите на экране: меню, кнопки, окна настроек, конвертер данных, окно запуска — всё здесь. Также здесь логика запуска, сортировка точек и расчёт времени. Это самый большой файл — около 1567 строк |
...
| [PIC.CPP](PIC.CPP) | **Файл управления оборудованием.** Здесь «живёт» вся работа с реальным железом: инициализация плат, перемещение столика, лазерные импульсы, контроль затворов и датчиков, основной цикл обработки (прохождение по всем точкам), программирование таймера PT2 и управление насосом/вентилем. Примерно 380 строк |
| [MSKEYC.CPP](MSKEYC.CPP) | **Файл работы с мышью и клавиатурой.** Инициализация мыши, обработка кликов, нажатий клавиш, автоповтор при удержании кнопки, управление курсором со стрелок клавиатуры, вывод часов, звуковой сигнал. Примерно 110 строк |
| [RDWRF.CPP](RDWRF.CPP) | **Файл чтения и записи данных.** Открытие/сохранение файлов всех форматов (BFT, CFT, ASC, BMP), работа с каталогами и дисками, чтение/запись конфигурации (11 полей, включая gate_delay, dT, sh_delay), загрузка каталога стёкол, работа со счётчиком вспышек. Примерно 417 строк |
...
| Файл | Зачем нужен |
|------|-------------|
| [FLINT_UTF8.txt](FLINT_UTF8.txt) | **Журнал изменений.** Описание что нового в каждой версии, какие ошибки исправлены |
| [FLINTOLD.TXT](FLINTOLD.TXT) | **Старый журнал** (версии 1.0–2.43, 1996–1998). Полная история с самого начала |
| [Flintold2.txt](Flintold2.txt) | **Средний журнал** (1999). Период внедрения на производстве в Минске (LOTIS TII) |

### Папка SCRIPTS/

| Файл | Зачем нужен |
|------|-------------|
| [verify_all.py](SCRIPTS/verify_all.py) | **Скрипт верификации** — автоматическая проверка соответствия исходных кодов |
...
| Z (вверх-вниз) | 0–12000 шагов | ~120 мм |

Максимальный размер стеклянного изделия — **235 × 235 × 120 мм**.
