...
- Позволяет редактировать файлы с координатами точек
...
## На какой операционной системе работает программа

**DOS** 

В 1996 году для управления оборудованием в реальном времени DOS подходил лучше — он не отвлекался на другие задачи и давал программе полный контроль над компьютером и подключёнными устройствами.

---

## Какое оборудование используется?

> Подробное описание плат, портов и электрической схемы: **[HARDWARE_DESCRIPTION.md](HARDWARE_DESCRIPTION.md)**

Программа управляет целым набором устройств, подключённых к компьютеру на Pentium 3, 128мб ОЗУ через специальные платы расширения:
...
### Лазер
Мощный импульсный лазер. Каждый «выстрел» создаёт одну точку внутри стекла. Программа управляет:
- **Интервалом между вспышками лампы накачки** (от 20 до 655 мс) - т.е. тем как часто лазер стреляет
- **Задержкой до открытия оптического затвора** - т.е. мощностью выстрела
- **Защитными затворами** — открывает и закрывает с помошью электромагнитов защитные шторки, которая блокирует луч, когда лазер не нужен

### Датчики безопасности
- **6 концевых датчиков** (по 1 на каждую границу осей X, Y, Z) — не дают столику «уехать» за пределы и сломать механику
- **10 датчиков контроля вспышек** — проверяют, что при каждом импульсе в стекле произошла вспышка
...
> Справочник всех функций с параметрами и диапазонами значений: **[FUNCTIONS_REFERENCE.md](FUNCTIONS_REFERENCE.md)**
...
- **Индикатор Dr.check** — показывает включена ли проверка выстрелов лазера
...
**Параметры лазера:**
- **Период лазера** (Las.puls period) — интервал между вспышками лампы накачки(от 20 до 655 мс)
- **Частота шагов** (Step freq) — максимальная разрешенная скорость перемещения столика (от 100 до 2000 Гц)
- **Режим скорости**: «постоянная» (constant) — столик всегда двигается с максимальной скоростью; «переменная» (variable) — скорость подстраивается так, чтобы лазер приходил в нужную точку одновременно по всем осям
...
- При включённом режиме «Work ptr» столик **реально перемещается** при каждом изменении координат — удобно для настройки точного положения
...
**Безопасность:**
- **Protect open** — ручное открытие/закрытие защитных эм-затворов лазера


**Управление лазером (Pump/Gate):**
- **Gate delay** — задержка срабатывания электро-оптического затвора лазера (×10 мкс, диапазон 10–500 мкс, максимум энергии - около 230 мкс)
...
**Загрузка данных:**
- Можно загрузить объект из **4 форматов**: BFT, CFT, ASC (3D Studio), BMP (чёрно-белое изображение)
...
Загрузка файла с точками **для гравировки**:
...
**Запуск процесса гравировки**. Окно с множеством настроек:
...
- **Drawing check** — если включено, после каждого импульса программа проверяет по датчикам, что каждый выстрел лазера действительно оставил разрушение в стекле (фотодиод зафиксировал вспышку). Если нет — до 11 повторных попыток
...
- Для каждой точки: вычисление координат → перемещение столика → лазерный импульс → отрисовка точки на контрольной картинке → инкремент CMOS-счётчика
...
> **Важно:** ось Y инвертируется при импорте и экспорте. В 3D Studio Y смотрит вверх, в FLINT Y — это физическая ось столика. При загрузке ASC программа автоматически умножает Y-координату на −1.

### BMP — монохромное изображение
Файл формата Windows Bitmap, строго **1-bit монохромный** (два цвета — чёрный и белый). Проверяется по размеру файла: заголовок должен быть ровно 62 байта (14 + 40 + 8 = BITMAPFILEHEADER + BITMAPINFOHEADER + палитра 2 цветов). 8-bit «серый» или любой другой BMP — не загружается. Чёрные пиксели превращаются в точки для гравировки. Можно задать шаг и глубину — получится плоское или двухслойное изображение внутри стекла.
...
| [cube.bft](cube.bft) | Простая геометрическая фигура — **куб**. Базовый тест |
| [sphere.bft](sphere.bft) | Геометрическая фигура — **сфера** |
| [spiral.bft](spiral.bft) | Геометрическая фигура — **спираль** |


### Документация (история изменений)

> Расшифровка форматов лог-файлов программы: **[LOG_REFERENCE.md](LOG_REFERENCE.md)**
...
| [FLINT_UTF8.txt](FLINT_UTF8.txt) | **Журнал изменений v2.61–2.77** (~120 строк). 8 версий (2003–2004): gate delay, таймер PT2, удаление часов, sh_delay |
...
5. **Настройка оборудования** (F1) — задать период лазера, частоту шагов, режим скорости, задержку перед выстрелом
...
9. **Завершение** — звуковой сигнал, надпись «TABLE END». Извлечь готовое изделие (F4 → Down).
...
1. **Оригинальные исходники версии 2.61** из папки `_GENESIS/Flint 2.61/SRC/` — как базовый код, из которого выросла v2.77
2. **Дизассемблированный код** оригинального `PROJ5.EXE` v2.77 — для верификации логики и выявления отличий от v2.61
3. **Журнал изменений** `_GENESIS/Flint 2.77/FLINT_UTF8.txt` — описание всех доработок от v2.61 до v2.77 (8 версий, 2003–2004)


### Сборка 16-битной версии

Скрипт `COMPILE.bat` автоматически определяет среду:
...
Папка `Flint 2.77 (WTCM32) src/` содержит **32-битный порт** программы, созданный для преодоления главного ограничения 16-битной версии — **нехватки памяти**.
...
## 32-битный порт (Borland C++ 32 / DPMI32)

Папка `Flint 2.77 (BCC32) src/` содержит альтернативный **32-битный порт** — на том же компиляторе Borland C++, но в его 32-битном варианте.

### Подход к портированию

В отличие от WTCM32-порта, где переход на Open Watcom потребовал написать полный слой совместимости (COMPAT.H) и собственную реализацию BGI (BGI32.CPP), BCC32-порт **остаётся внутри семейства Borland**. Компилятор `BCC32.EXE` понимает тот же API, что и 16-битный `BCC.EXE` — не нужно переименовывать функции или менять структуры. Вместо этого потребовалось устранить несколько специфических проблем самого BCC32:

- **`#undef inportb/outportb`** — флаг `-WX` (целевая платформа: DOS extended) вынуждает BCC32 считать себя Win32-компилятором, что превращает `inportb/outportb` из `<dos.h>` в нерабочие макросы. Их отключение (`#undef`) возвращает вызов к реальным функциям из `DPMI32.LIB`.

- **`biosdisk()` заблокирован** — Borland обрамил `biosdisk()` в `<bios.h>` условием `#if !defined(_Windows)`. Поскольку `-WX` определяет `_Windows`, функция становится недоступна. В `PROJ5.H` она переопределена вручную через `int386x(0x13)`.

- **Пользовательский курсор мыши работает** — в отличие от WTCM32, где передача far-указателя в `int 33h/AX=9` вызывала GPF, в BCC32 используется `int386(0x33)` с полным набором 32-битных регистров, и курсор устанавливается корректно.

### Компилятор и сборка

- **Компилятор**: `BCC32.EXE -WX -O2 -3` (целевая платформа DOS extended, оптимизация по скорости)
- **Расширитель памяти**: DPMI32 (собственный Borland 32-bit protected mode)
- **Рантайм**: `32RTM.EXE` + `DPMI32VM.OVL` — аналог DOS/4GW от Borland
- **BGI**: `BGI32.DLL` + `BGIVGA.DLL` (Borland Power Pack for DOS 1.0a) — те же функции BGI, но поставляемые как DLL, а не как встроенный драйвер
- **Сборка**: `COMPILE.bat` (только Windows NT+; `BCC32.EXE` — это Win32-приложение, в DOS не запускается)

### Изменения в исходных файлах

Новых файлов не создавалось — все изменения внесены в существующие файлы:

| Аспект | 16-bit (Borland) | 32-bit (BCC32) | Где изменено |
|--------|-------------------|----------------|-------------|
| Массив точек | `int huge *iTabl` | `short *iTabl` | PROJ5.H |
| Доступ к точкам | Far-указатели с ручной сегментной арифметикой | Прямая индексация `iTabl[3*i]` | PROJ5.CPP |
| Аллокатор | `farmalloc(farcoreleft()-30000)` | `malloc()` (каскадный: 8M → 2M → 400K) | PROJ5.CPP |
| BGI инициализация | `registerfarbgidriver()` + `initgraph("","")` | `initgraph(bgi_path,"")` где path → `BGI_DRVR\` | PROJ5.CPP |
| Порты ввода-вывода | `inportb/outportb` (работают) | `#undef` + реальные функции DPMI32.LIB | PROJ5.H |
| `biosdisk()` | Из `<bios.h>` | Самописная реализация через `int386x(0x13)` | PROJ5.H |
| Мышиный курсор | `geninterrupt(0x33)` + битовая маска | `int386(0x33)` — полноценный, не заглушка | PROJ5.H, MSKEYC.CPP |
| Буферы изображений | `const far *bitmapmsg[2000]`; `bgmap[200]` | `void *bitmapmsg`; `void *bgmap` (heap) | PROJ5.H, PROJ5.CPP |
| Модификаторы `far`/`huge` | Повсеместно | Удалены (плоская модель памяти) | Все файлы |

> **Почему не нужен COMPAT.H?** В WTCM32-порте слой совместимости нужен потому, что Open Watcom использует другие имена функций (`outp` вместо `outportb`, `_getdrive` вместо `getdisk` и т.д.). BCC32 — это тот же Borland, только 32-битный. Имена и сигнатуры функций совпадают, нужно лишь устранить специфические баги компилятора.
...
| Документ | Содержание |
|----------|------------|
| [HARDWARE_DESCRIPTION.md](HARDWARE_DESCRIPTION.md) | Схема оборудования, адреса портов, описание плат расширения |
| [FUNCTIONS_REFERENCE.md](FUNCTIONS_REFERENCE.md) | Справочник всех функций программы с параметрами и диапазонами |
| [LOG_REFERENCE.md](LOG_REFERENCE.md) | Форматы лог-файлов, описание полей и расшифровка записей |
| [FLINT_Help.html](FLINT_Help.html) | Встроенная справка программы (оригинальный HTML) |
