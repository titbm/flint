...
## 1. PROJ5.CPP — Главный модуль (~1567 строк)
...
**Рисует сетку 3×3** с реальными счётчиками реитераций для каждой позиции стекла. Параметр `mode`: 0 — только метки, 1 — подсветка активных позиций цветом `COL_ON`. Используется переменная `rpt_active` для индикации текущей позиции. Отображается на главном экране и обновляется в реальном времени во время обработки.
...
## 2. PIC.CPP — Модуль управления оборудованием (~380 строк)
...
2. Рассчитывает K2 (делитель частоты) для каждой оси:
   - В режиме variable: K2 = (l_period-**dT**)*100/|d| (чтобы перемещение заняло ~l_period мс). **Константа `3` заменена переменной `dT`** для калибровки под конкретный компьютер
...
### `void read_pt2(void)`
**Читает состояние таймера PT2** для синхронизации лазерных импульсов. Используется в `lpuls()` вместо прямого управления портами.

---

## 3. MSKEYC.CPP — Модуль ввода (~110 строк)
...
#### `void WrCfg(int gl_nmber)`
**Записывает файл конфигурации** (flint1.cfg или flint9.cfg). Записывает 10–11 полей (аналогично RdCfg): Xptr, Yptr, Zptr, Step, l_period, gate_delay, step_freq, f_const, dT, sh_delay [, glpos9].
...
### Константы и макросы
- **Адреса портов**: ADRPT0 (0x300), ADRPPI (0x308), ADRRG (0x30C), ADRCMOS (0x318), **ADRPT1 (0x310)**, **ADRPPI2 (0x314)**, **ADRPT2 (0x318)**, **LRG (0x31C)**
...
- `rpt_active` (int) — битовая маска активных позиций для `show_reit()`
